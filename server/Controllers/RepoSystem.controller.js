...
export const createRepo = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("SELECT * FROM repos WHERE repo_name = ?", [
      repoName,
    ]);
    ...
    if (rows.length > 0) {
      return res.status(400).json({
        success: false,
        errorCode: "REPO_FOUND",
        message: "Repository already exists",
      });
    }

    const [result] = await pool.query(
      "INSERT INTO repos (user_id, repo_name) VALUES (?, ?)",
      [userId, repoName],
    );
...
export const getAllRepos = async (req, res) => {
  try {
    const { id } = req.user;
    const [repos] = await pool.query("SELECT * FROM repos where user_id = ?", [
      id,
    ]);
...
export const addRepo = async (req, res) => {
  try {
    ...
    if (!repoId || isNaN(Number(repoId))) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid repoId" });
    }
    ...
    if (!paths || !Array.isArray(paths) || paths.length !== files.length) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid file paths provided" });
    }

    const [rows] = await pool.query("SELECT * FROM repos WHERE id = ?", [
      repoId,
    ]);
    const repo = rows[0];
    if (!repo) {
    ...
    const uploadResults = await Promise.allSettled(
      files.map((file, idx) => {
        ...
        return uploadToS3(s3Key, file.buffer, s3Client, file.size);
      }),
    );
